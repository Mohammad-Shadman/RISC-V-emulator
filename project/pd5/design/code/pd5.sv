...
module pd5 #(
    ...
    resetReg #(.DWIDTH(1))pcsel_reg_de_rr(.clk(clk), .reset(reset),
    .data_i(pcsel_control), .data_o(pcsel_reg_de));

    resetReg #(.DWIDTH(1))regwren_reg_de_rr(.clk(clk), .reset(reset),
    .data_i(regwren_control), .data_o(regwren_reg_de));

    resetReg #(.DWIDTH(1))regwren_reg_em_rr(.clk(clk), .reset(reset),
    .data_i(regwren_reg_de), .data_o(regwren_reg_em));

    resetReg #(.DWIDTH(1))regwren_reg_mw_rr(.clk(clk), .reset(reset),
    .data_i(regwren_reg_em), .data_o(regwren_reg_mw));

    resetReg #(.DWIDTH(1))rs1sel_reg_de_rr(.clk(clk), .reset(reset),
    .data_i(rs1sel_reg_em), .data_o(rs1sel_reg_mw));

    resetReg #(.DWIDTH(1))rs2sel_reg_de_rr(.clk(clk), .reset(reset),
    .data_i(rs2sel_control), .data_o(rs2sel_reg_de));

    resetReg #(.DWIDTH(1))memwren_reg_de_rr(.clk(clk), .reset(reset),
    .data_i(memwren_control), .data_o(memwren_reg_de));

    resetReg #(.DWIDTH(1))memwren_reg_em_rr(.clk(clk), .reset(reset),
    .data_i(memwren_reg_de), .data_o(memwren_reg_em));

    resetReg #(.DWIDTH(2))wbsel_reg_de_rr(.clk(clk), .reset(reset),
    .data_i(wbsel_control), .data_o(wbsel_reg_de));

    resetReg #(.DWIDTH(2))wbsel_reg_em_rr(.clk(clk), .reset(reset),
    .data_i(wbsel_reg_de), .data_o(wbsel_reg_em))

    resetReg #(.DWIDTH(2))wbsel_reg_mw_rr(.clk(clk), .reset(reset),
    .data_i(wbsel_reg_em), .data_o(wbsel_reg_mw))

    resetReg #(.DWIDTH(4))alusel_reg_de_rr(.clk(clk), .reset(reset),
    .data_i(alusel_control), .data_o(alusel_reg_de));

    resetReg #(.DWIDTH(2))memsel_reg_de_rr(.clk(clk), .reset(reset),
    .data_i(memsel_control), .data_o(memsel_reg_de));

    resetReg #(.DWIDTH(2))memsel_reg_em_rr(.clk(clk), .reset(reset),
    .data_i(memsel_reg_de), .data_o(memsel_reg_de));

    resetReg #(.DWIDTH(AWIDTH)) pc_reg_de_rr (.clk(clk), .reset(reset),
    .data_i(pc_decode), .data_o(pc_reg_de));

    resetReg #(.DWIDTH(DWIDTH)) insn_reg_de_rr (.clk(clk), .reset(reset),
    .data_i(insn_decode), .data_o(insn_reg_de));

    resetReg #(.DWIDTH(7)) opcode_reg_de_rr (.clk(clk), .reset(reset),
    .data_i(opcode_decode), .data_o(opcode_reg_de));

    resetReg #(.DWIDTH(5)) rd_reg_de_rr (.clk(clk), .reset(reset),
    .data_i(rd_decode), .data_o(rd_reg_de));

    resetReg #(.DWIDTH(5)) rs1_reg_de_rr (.clk(clk), .reset(reset),
    .data_i(rs1_decode), .data_o(rs1_reg_de));

    resetReg #(.DWIDTH(5)) rs2_reg_de_rr (.clk(clk), .reset(reset),
    .data_i(rs2_decode), .data_o(rs2_reg_de));

    resetReg #(.DWIDTH(7)) funct7_reg_de_rr (.clk(clk), .reset(reset),
    .data_i(funct7_decode), .data_o(funct7_reg_de));

    resetReg #(.DWIDTH(3)) funct3_reg_de_rr (.clk(clk), .reset(reset),
    .data_i(funct3_decode), .data_o(funct3_reg_de));

    resetReg #(.DWIDTH(5)) shamt_reg_de_rr (.clk(clk), .reset(reset),
    .data_i(shamt_decode), .data_o(shamt_reg_de));

    resetReg #(.DWIDTH(DWIDTH)) imm_reg_de_rr (.clk(clk), .reset(reset),
    .data_i(imm_decode), .data_o(imm_reg_de));

    resetReg #(.DWIDTH(DWIDTH)) rs1data_reg_de_rr(.clk(clk), .reset(reset),
    .data_i(rs1data_rf), .data_o(rs1data_reg_de));

    resetReg #(.DWIDTH(DWIDTH)) rs2data_reg_de_rr(.clk(clk), .reset(reset),
    ...
    resetReg #(.DWIDTH(DWIDTH)) res_reg_em_rr(.clk(clk), .reset(reset),
    ...
    always_comb begin
        ...
        Bsel  = rs2sel_control ? rs2data_rf : imm_decode;
    end
